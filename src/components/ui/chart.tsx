      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item: any, index: number) => {
            if (!item) return null;
            const key = `${nameKey || (item?.name as string | number) || (item?.dataKey as string | number) || "value"}`
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            const indicatorColor = color || (item?.payload && typeof item.payload === 'object' && 'fill' in item.payload ? item.payload.fill : null) || item?.color
    ...
    return (
      ...
      >
        {payload.map((item: any) => {
          const key = `${nameKey || (item?.dataKey as string | number) || "value"}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
